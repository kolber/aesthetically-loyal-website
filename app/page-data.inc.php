...
Class PageData {
	...
	static function create_columns($page) {
	  $columns = array(array(), array(), array());
    
    $i = 0;
	  foreach($page->data['$children'] as $child) {
	    $columns[($i % 3)][] = $child;
	    $i++;
	  }
	...
	static function create_collections($page) {
	  # $root
		...
		# $columns
		$columns = self::create_columns($page);
		foreach($columns as $key => $column) eval('$page->column'.($key + 1).' = $column;');
	}
